...
import {
  Folder, HardDrive, Shield, Copy, Sparkles, Cloud, Smartphone, Tablet, Monitor,
  RotateCw, Maximize2, FileCheck, CheckCircle2, AlertTriangle, Search, Lock, Plus,
  RefreshCw, Crosshair, Touchpad, Menu, X, Sliders, Zap,
} from 'lucide-react';
...
import {
  fetchGoogleDriveData, moveGoogleDriveFile, createGoogleDriveFolder, deleteGoogleDriveFile, DriveFileTypeFilter,
...
export default function App() {
  ...
  const [activeViewMode, setActiveViewMode] = useState<'emulator' | 'native' | 'audit_report'>('native');
  ...
  const [userProfile, setUserProfile] = useState({
    name: 'Awasthi Sach', email: 'awasthi.sach@gmail.com',
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (result) {
        ...
        showDriveToast('Google Sign-In window closed.');
      }
    } catch (err: any) {
  ...
  const handleDeleteFile = async (id: string) => {
    ...
    if (token && fileToDelete?.isGoogleDriveItem) {
      try {
        ...
        showDriveToast(`"${fileToDelete.name}" deleted`);
      } catch (err) {
        console.error(err);
      }
    }
    ...
    if (token) {
      ...
      if (driveItems.length > 0) {
        try {
          ...
          showDriveToast(`${driveItems.length} file(s) removed`);
        } catch (err) {
          console.error(err);
        }
      }
